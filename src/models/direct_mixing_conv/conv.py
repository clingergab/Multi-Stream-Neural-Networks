...
class _DMConvNd(nn.Module):
    ...
    integrated_weight: Tensor  # Conv1x1 weight for integrated pathway
    ...
    def __init__(
        ...
        # Create scalar mixing weights for Direct Mixing + Conv
        # Formula: integrated_l = Σ(α_i · stream_i_l) + γ · Conv1x1(integrated_{l-1}) + b_integrated
    ...
    def reset_parameters(self) -> None:
        ...
        # Initialize integrated pathway weights (Conv1x1)
...
class DMConv2d(_DMConvNd):
    ...
    def _conv_forward(
        ...
        """
        Forward pass with Direct Mixing + Conv integration.

        Biological analogy (Middle Ground - Most Biologically Plausible):
        ...
        - Synaptic weights: Conv1x1 models variable synaptic strengths from integrated_prev
        - Soma threshold: ReLU applied in blocks.py (firing threshold)

        Key architecture (biologically accurate):
        1. Conv1x1 on integrated_prev: Models synaptic weight diversity
        2. NO BN on integrated: Raw membrane potential (no artificial normalization)
        3. Integrated stream: Conv1x1(prev) + stream mixing → ReLU
        """
        ...
        # Key features: Conv1x1 for synaptic plasticity, NO BN (raw membrane potential)
        ...
        if integrated_input is not None:
            ...
            # Apply scalar weight to Conv1x1 output (controls recurrent connection strength)
        ...
        # Return integrated output (raw membrane potential, no BN)
        # ReLU will be applied in blocks.py for spiking threshold
    ...
    def forward(self, stream_inputs: list[Tensor], integrated_input: Optional[Tensor] = None) -> tuple[list[Tensor], Tensor]:
        """Forward pass through all N convolution streams with Direct Mixing + Conv."""
...
class _DMNormBase(nn.Module):
    ...
    # NOTE: No integrated BN buffers for direct_mixing_conv (integrated gets GroupNorm(1), no BN)
    ...
    def __init__(
        ...
        if self.affine:
            ...
            # Note: No integrated_weight/bias for direct_mixing_conv (integrated gets GroupNorm)
        else:
        ...
        self.integrated_groupnorm = nn.GroupNorm(1, integrated_num_features, eps=eps, affine=affine, **factory_kwargs)
        ...
        if self.track_running_stats:
            ...
            # Note: No integrated_running_mean/var for direct_mixing_conv (integrated gets GroupNorm)
            ...
            # Note: No integrated buffers for direct_mixing_conv
    ...
    def extra_repr(self):
        ...
        return (
            f"num_streams={self.num_streams}, "
            f"stream_num_features={self.stream_num_features} (BatchNorm), "
            f"integrated_num_features={self.integrated_num_features} (GroupNorm(1)), "
...
class _DMBatchNorm(_DMNormBase):
    ...
    def forward(self, stream_inputs: list[Tensor], integrated_input: Optional[Tensor] = None) -> tuple[list[Tensor], Tensor]:
        ...
        # Process integrated pathway using GroupNorm(1) = LayerNorm over channels
...
class DMBatchNorm2d(_DMBatchNorm):
    ...
    For direct_mixing_conv: This layer applies Batch Normalization to stream pathways and
