...
class DMNet(BaseModel):
    ...
    def _initialize_weights(self, zero_init_residual: bool):
        ...
        for m in self.modules():
            if isinstance(m, DMConv2d):
                ...
                # DMBatchNorm2d parameters already initialized in _DMNormBase.reset_parameters()
                pass

        # Zero-initialize the last BN in each residual branch,
        # so that the residual branch starts with zeros, and each residual block behaves like an identity.
        # This improves the model by 0.2~0.3% according to https://arxiv.org/abs/1706.02677
        # Note: For direct_mixing_activation, we only have stream BN weights (no integrated BN)
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, DMBottleneck):
                    # Zero-init last BN weight for each stream pathway
                    for i, stream_weight in enumerate(m.bn3.stream_weights):
                        nn.init.constant_(stream_weight, 0)
                elif isinstance(m, DMBasicBlock):
                    # Zero-init last BN weight for each stream pathway
                    for i, stream_weight in enumerate(m.bn2.stream_weights):
                        nn.init.constant_(stream_weight, 0)
    ...
    def analyze_pathway_weights(self) -> dict:
        """
        Analyze the weight distributions and magnitudes across all N stream pathways.

        For direct_mixing_activation:
        - Stream weights: Full convolution kernels
        - Scalar mixing weights: α_i (stream contribution), γ (recurrent connection)
        - No integrated_weight (we use scalar γ instead of Conv1x1)

        Returns:
            Dictionary containing weight analysis for all stream pathways and mixing scalars
        """
        # Initialize dictionaries for N streams
        stream_weights = [{} for _ in range(self.num_streams)]
        scalar_mixing_weights = {}

        # Analyze DM layers - look for modules with stream_weights
        for name, module in self.named_modules():
            if hasattr(module, 'stream_weights'):
                # DMConv2d modules
        ...
        # Add scalar mixing weights (α and γ)
        result['scalar_mixing_weights'] = scalar_mixing_weights

        # Ratio analysis - compare streams pairwise
        ratio_analysis = {}
        ...
        # Per-layer ratios (stream comparisons)
        layer_ratios = {}
        ...
        result['note'] = (
            'For direct_mixing_activation: No integrated_weight (using scalar γ instead). '
            'Integration uses scalar mixing: integrated = Σ(α_i · stream_i) + γ · integrated_prev'
        )
    ...
    def calculate_stream_contributions_to_integration(self, data_loader: DataLoader = None):
        ...
        For direct_mixing_activation, this analyzes scalar mixing weights (α_i):
        - Each stream has a learned scalar α_i that controls its contribution
        - Integration formula: integrated = Σ(α_i · stream_i) + γ · integrated_prev

        This directly answers: "How much does the model architecture favor each stream?"

        Simple, fast, and interpretable:
        - No data required (analyzes learned scalar weights)
        ...
        # Sum absolute values of α_i across all layers to get total contribution magnitude
        ...
        result['note'] = (
            'Measures architectural contribution based on scalar mixing weights (α_i). '
            f'Reflects how the trained model structurally combines the {self.num_streams} input streams using scalar multiplication.'
        )
...
# Factory functions for common DMNet architectures
