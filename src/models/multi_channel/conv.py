"""
Multi-Channel convolution layers for separate stream processing.

This module provides dual-stream convolution operations that maintain
independent pathways throughout the network architecture.
"""
...
class _MCConvNd(nn.Module):
    ...
    _ConvNd but adapted for dual-stream (stream1/stream2) processing.
    """
    
    __constants__ = [
        "stride", "padding", "dilation", "groups", "padding_mode", "output_padding",
        "stream1_in_channels", "stream2_in_channels", 
        "stream1_out_channels", "stream2_out_channels", "kernel_size",
    ]
    __annotations__ = {
    ...
    def _conv_forward(self, stream1_input: Tensor, stream2_input: Tensor, 
                     stream1_weight: Tensor, stream2_weight: Tensor,
                     stream1_bias: Optional[Tensor], stream2_bias: Optional[Tensor]) -> tuple[Tensor, Tensor]:
...
class MCConv2d(_MCConvNd):
    ...
    This layer processes stream1 and stream2 streams separately using dual weight/bias
    ...
    def forward(self, stream1_input: Tensor, stream2_input: Tensor) -> tuple[Tensor, Tensor]:
        ...
        return self._conv_forward(
            stream1_input, stream2_input, self.stream1_weight, self.stream2_weight,
            self.stream1_bias, self.stream2_bias
        )
    ...
    def _forward_interleaved(self, stream1_input: Tensor, stream2_input: Tensor) -> tuple[Tensor, Tensor]:
...
class _MCNormBase(nn.Module):
    ...
    _version = 2
    __constants__ = ["track_running_stats", "momentum", "eps", "stream1_num_features", "stream2_num_features", "affine"]
...
class _MCBatchNorm(_MCNormBase):
    ...
    def __init__(
        ...
        factory_kwargs = {"device": device, "dtype": dtype}
        super().__init__(
            stream1_num_features, stream2_num_features, eps, momentum, affine, track_running_stats, **factory_kwargs
        )
...
class MCBatchNorm2d(_MCBatchNorm):
    ...
    This layer applies Batch Normalization separately to color and stream2 pathways,
