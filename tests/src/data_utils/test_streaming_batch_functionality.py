...
class TestBatchLoadingFunctionality:
    ...
    def test_validation_dataloader_batch_behavior(self, large_temp_dataset):
        ...
        # Validation loader should use batch_size * 2 = 8
        val_batch = next(iter(val_loader))
        rgb, brightness, labels = val_batch
        
        # Should be larger batch size for validation
        assert rgb.shape[0] == 8 or rgb.shape[0] == 20  # 8 or all samples if < 8 remaining
