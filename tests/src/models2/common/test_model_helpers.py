...
from src.models2.common.model_helpers import (
    setup_scheduler,
    update_scheduler,
...
class TestSetupScheduler:
    ...
    def test_setup_scheduler_none(self):
        ...
        model = MockModel()
        setup_scheduler(model, epochs=10, train_loader_len=100)
        assert model.scheduler is None
    
    def test_setup_scheduler_cosine(self):
        """Test cosine annealing scheduler setup."""
        model = MockModel(scheduler_type='cosine')
        setup_scheduler(model, epochs=10, train_loader_len=100)
        assert isinstance(model.scheduler, CosineAnnealingLR)
        assert model.scheduler.T_max == 10
    
    def test_setup_scheduler_cosine_custom_t_max(self):
        """Test cosine annealing scheduler with custom t_max."""
        model = MockModel(scheduler_type='cosine')
        setup_scheduler(model, epochs=10, train_loader_len=100, t_max=15)
        assert isinstance(model.scheduler, CosineAnnealingLR)
        assert model.scheduler.T_max == 15
    
    def test_setup_scheduler_onecycle(self):
        """Test OneCycle scheduler setup."""
        model = MockModel(scheduler_type='onecycle')
        setup_scheduler(model, epochs=10, train_loader_len=100)
        assert isinstance(model.scheduler, OneCycleLR)
        assert model.scheduler.total_steps == 1000  # 10 * 100
    
    def test_setup_scheduler_onecycle_custom_params(self):
        """Test OneCycle scheduler with custom parameters."""
        model = MockModel(scheduler_type='onecycle')
        setup_scheduler(
            model, epochs=5, train_loader_len=50,
            max_lr=0.1, pct_start=0.2, anneal_strategy='linear',
            div_factor=10.0, final_div_factor=100.0
        )
        assert isinstance(model.scheduler, OneCycleLR)
        assert model.scheduler.total_steps == 250  # 5 * 50
    
    def test_setup_scheduler_step(self):
        """Test StepLR scheduler setup."""
        model = MockModel(scheduler_type='step')
        setup_scheduler(model, epochs=10, train_loader_len=100)
        assert isinstance(model.scheduler, StepLR)
        assert model.scheduler.step_size == 30
        assert model.scheduler.gamma == 0.1
    
    def test_setup_scheduler_step_custom_params(self):
        """Test StepLR scheduler with custom parameters."""
        model = MockModel(scheduler_type='step')
        setup_scheduler(model, epochs=10, train_loader_len=100, step_size=20, gamma=0.5)
        assert isinstance(model.scheduler, StepLR)
        assert model.scheduler.step_size == 20
        assert model.scheduler.gamma == 0.5
    
    def test_setup_scheduler_plateau(self):
        """Test ReduceLROnPlateau scheduler setup."""
        model = MockModel(scheduler_type='plateau')
        setup_scheduler(model, epochs=10, train_loader_len=100)
        assert isinstance(model.scheduler, ReduceLROnPlateau)
        assert model.scheduler.patience == 10
        assert model.scheduler.factor == 0.5
    
    def test_setup_scheduler_plateau_custom_params(self):
        """Test ReduceLROnPlateau scheduler with custom parameters."""
        model = MockModel(scheduler_type='plateau')
        setup_scheduler(
            model, epochs=10, train_loader_len=100,
            scheduler_patience=5, factor=0.2
        )
        assert isinstance(model.scheduler, ReduceLROnPlateau)
        assert model.scheduler.patience == 5
        assert model.scheduler.factor == 0.2
    
    def test_setup_scheduler_plateau_fallback_patience(self):
        """Test ReduceLROnPlateau scheduler with fallback patience parameter."""
        model = MockModel(scheduler_type='plateau')
        setup_scheduler(model, epochs=10, train_loader_len=100, patience=15)
        assert isinstance(model.scheduler, ReduceLROnPlateau)
        assert model.scheduler.patience == 15
    
    def test_setup_scheduler_invalid_type(self):
        """Test that invalid scheduler type raises ValueError."""
        model = MockModel(scheduler_type='invalid')
...
class TestUpdateScheduler:
    ...
    def test_update_scheduler_none(self):
        ...
        model = MockModel()
        update_scheduler(model, val_loss=0.5)  # Should not raise
    ...
    def test_update_scheduler_plateau(self):
        ...
        with patch.object(model.scheduler, 'step') as mock_step:
            update_scheduler(model, val_loss=0.5)
...
class TestPrintEpochProgress:
    ...
    def test_print_epoch_progress_with_validation(self, capsys):
        ...
        model = MockModel()
        print_epoch_progress(
            model, epoch=0, epochs=10, epoch_time=12.34,
    ...
    def test_print_epoch_progress_without_validation(self, capsys):
        ...
        model = MockModel()
        print_epoch_progress(
            model, epoch=4, epochs=10, epoch_time=15.67,
...
class TestSaveCheckpoint:
    ...
    def test_save_checkpoint_with_history(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            checkpoint_path = os.path.join(temp_dir, 'checkpoint.pth')
            save_checkpoint(model, checkpoint_path, history=history)
    ...
    def test_save_checkpoint_no_optimizer(self):
        """Test checkpoint saving when model has no optimizer."""
        model = MockModel()
    ...
    def test_save_checkpoint_no_scheduler(self):
        """Test checkpoint saving when model has no scheduler."""
        model = MockModel()
...
class TestCreateDataloaderFromTensors:
    ...
    def test_create_dataloader_cuda(self):
        """Test DataLoader creation for CUDA device."""
        model = MockModel(device='cuda')
        X = torch.randn(100, 10)
        y = torch.randint(0, 2, (100,))
        
        dataloader = create_dataloader_from_tensors(model, X, y, batch_size=16, shuffle=True)
    ...
    def test_create_dataloader_mps(self):
        """Test DataLoader creation for MPS device."""
        model = MockModel(device='mps')
        X = torch.randn(100, 10)
        y = torch.randint(0, 2, (100,))
        
        dataloader = create_dataloader_from_tensors(model, X, y, batch_size=32)
    ...
    def test_create_dataloader_cpu(self):
        """Test DataLoader creation for CPU device."""
        model = MockModel(device='cpu')
        X = torch.randn(100, 10)
        y = torch.randint(0, 2, (100,))
        
        dataloader = create_dataloader_from_tensors(model, X, y, batch_size=8)
    ...
    def test_create_dataloader_no_targets(self):
        ...
        dataloader = create_dataloader_from_tensors(model, X, batch_size=16)
    ...
    def test_create_dataloader_with_targets(self):
        ...
        dataloader = create_dataloader_from_tensors(model, X, y, batch_size=16)
...
class TestSetupEarlyStopping:
    ...
    def test_setup_early_stopping_disabled(self):
        ...
        config = setup_early_stopping(
            model, early_stopping=False, val_loader=val_loader,
    ...
    def test_setup_early_stopping_no_val_loader(self, capsys):
        ...
        config = setup_early_stopping(
            model, early_stopping=True, val_loader=None,
    ...
    def test_setup_early_stopping_val_loss_monitor(self, capsys):
        ...
        config = setup_early_stopping(
            model, early_stopping=True, val_loader=val_loader,
...
class TestEarlyStoppingInitiated:
    ...
    def test_early_stopping_initiated_disabled(self):
        ...
        should_stop = early_stopping_initiated(
            model, early_stopping_state, val_loss=0.5, val_acc=0.8,
    ...
    def test_early_stopping_initiated_triggered(self):
        ...
        with patch('src.models2.common.model_helpers.finalize_early_stopping') as mock_finalize:
            should_stop = early_stopping_initiated(
                model, early_stopping_state, val_loss=0.5, val_acc=0.8,
                epoch=5, pbar=None, verbose=True, restore_best_weights=True
            )
    ...
    def test_early_stopping_initiated_val_accuracy_improvement(self):
        ...
        should_stop = early_stopping_initiated(
            model, early_stopping_state, val_loss=0.5, val_acc=0.85,
            epoch=5, pbar=None, verbose=False, restore_best_weights=False
        )
...
class TestFinalizeEarlyStopping:
    """Test finalize_early_stopping function."""
    
    def test_finalize_early_stopping_val_loss(self, capsys):
        """Test finalize early stopping with val_loss monitor."""
        model = MockModel()
        early_stopping_state = {
            'monitor': 'val_loss',
            'best_metric': 0.3,
        ...
        finalize_early_stopping(
            model, early_stopping_state, epoch=10, pbar=None,
            verbose=True, restore_best_weights=False
        )
        
        captured = capsys.readouterr()
        assert "Early stopping triggered after 11 epochs" in captured.out
        assert "Best val_loss: 0.3000 at epoch 6" in captured.out
    
    def test_finalize_early_stopping_restore_weights(self, capsys):
        """Test finalize early stopping with weight restoration."""
        model = MockModel()
        best_weights = {'param': torch.randn(5, 5)}
        early_stopping_state = {
            'monitor': 'val_accuracy',
        ...
        finalize_early_stopping(
            model, early_stopping_state, epoch=15, pbar=None,
            verbose=True, restore_best_weights=True
        )
        
        captured = capsys.readouterr()
        assert "Early stopping triggered after 16 epochs" in captured.out
        assert "Best val_accuracy: 0.9500 at epoch 9" in captured.out
        assert "Restored best model weights" in captured.out
    
    def test_finalize_early_stopping_no_weights(self):
        """Test finalize early stopping with no saved weights."""
        model = MockModel()
        early_stopping_state = {
            'monitor': 'val_loss',
            'best_metric': 0.4,
        ...
        finalize_early_stopping(
            model, early_stopping_state, epoch=8, pbar=None,
            verbose=False, restore_best_weights=True
        )
        
        # Should not raise any errors
    
    def test_finalize_early_stopping_with_pbar(self):
        """Test finalize early stopping with progress bar (no verbose output)."""
        model = MockModel()
        early_stopping_state = {
            'monitor': 'val_loss',
            'best_metric': 0.4,
        ...
        finalize_early_stopping(
            model, early_stopping_state, epoch=8, pbar=pbar,
            verbose=True, restore_best_weights=False
        )
        
        # Should not print anything when pbar is provided
...
class TestCreateProgressBar:
    ...
    def test_create_progress_bar_verbose(self):
        ...
        with patch('src.models2.common.model_helpers.tqdm') as mock_tqdm:
            mock_pbar = Mock()
            mock_tqdm.return_value = mock_pbar
            
            result = create_progress_bar(
                model, verbose=True, epoch=5, epochs=10, total_steps=100
            )
    ...
    def test_create_progress_bar_not_verbose(self):
        ...
        result = create_progress_bar(
            model, verbose=False, epoch=5, epochs=10, total_steps=100
        )
...
class TestFinalizeProgressBar:
    ...
    def test_finalize_progress_bar_none(self):
        ...
        # Should not raise any errors
        finalize_progress_bar(
            model, pbar=None, avg_train_loss=0.5, train_accuracy=0.8,
    ...
    def test_finalize_progress_bar_no_validation(self):
        ...
        finalize_progress_bar(
            model, pbar=mock_pbar, avg_train_loss=0.5, train_accuracy=0.8,
            val_loader=None, val_loss=0.0, val_acc=0.0,
    ...
    def test_finalize_progress_bar_with_validation(self):
        ...
        finalize_progress_bar(
            model, pbar=mock_pbar, avg_train_loss=0.3, train_accuracy=0.9,
...
class TestUpdateHistory:
    ...
    def test_update_history_basic(self):
        ...
        update_history(
            model, history, avg_train_loss=0.5, train_accuracy=0.8,
    ...
    def test_update_history_with_validation(self):
        ...
        update_history(
            model, history, avg_train_loss=0.3, train_accuracy=0.9,
...
class TestIntegration:
    ...
    def test_full_early_stopping_workflow(self):
        ...
        should_stop = early_stopping_initiated(
            model, early_stopping_state, val_loss=0.6, val_acc=0.7,
        ...
        should_stop = early_stopping_initiated(
            model, early_stopping_state, val_loss=0.7, val_acc=0.6,
    ...
    def test_scheduler_and_dataloader_integration(self):
        ...
        model = MockModel(scheduler_type='cosine', device='cpu')
        
        # Setup scheduler
        setup_scheduler(model, epochs=5, train_loader_len=50)
        assert isinstance(model.scheduler, CosineAnnealingLR)
        
        # Create dataloader
        X = torch.randn(100, 10)
        y = torch.randint(0, 2, (100,))
        dataloader = create_dataloader_from_tensors(model, X, y, batch_size=20)
